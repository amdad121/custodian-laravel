...
The `Gate::before` hook no longer grants checks that pass arguments. `$user->can('posts.edit', $post)` now goes straight to your `PostPolicy` (or a `Gate::define`), even if the user has the `posts.edit` permission.
...
- **Action:** if a listener reads `RoleAssigned::$roleIds` or `PermissionGranted::$permissionIds` after a sync, expect only the newly attached IDs. The events are not dispatched when nothing changed.
